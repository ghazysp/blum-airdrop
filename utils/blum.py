...
class BlumBot:
    ...
    async def claim_task(self, task: dict):
        ...
        logger.debug(f"{self.client.name} |klaim sesi: {resp_json}")
    ...
    async def start_complete_task(self, task: dict):
        ...
        logger.debug(f"{self.client.name} | start sesi: {resp_json}")
    ...
    async def get_tasks(self):
        ...
        logger.debug(f"{self.client.name} | Get sesi: {resp_json}")

        # Ensure the response is a list of tasks
        if isinstance(resp_json, list):
            return resp_json
        else:
            logger.error(f"{self.client.name} | Salah format pada sesi: {resp_json}")
            return []
    ...
    async def play_game(self, play_passes: int):
        ...
        while play_passes:
            try:
                ...
                if not game_id or game_id == "game ngga jalan":
                    logger.info(f"{self.client.name} | tiket tersedia: {play_passes}")
                    break
                ...
                if isinstance(msg, bool) and msg:
                    logger.info(f"{self.client.name} | Game selesai bos; reward: {points}")
                else:
                    logger.info(f"{self.client.name} | Game tidak jalan; msg: {msg} tiket tersedia: {play_passes}")
                    break
                ...
                logger.error(f"{self.client.name} | Eror pada game: {e}")
