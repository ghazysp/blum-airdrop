                ...
                while True:
                    try:
                        ...
                        if isinstance(msg, bool) and msg:
                            logger.success(f"{account} |Done klaim daily")
                        ...
                        if claim_amount != 0 and is_available:
                            ...
                            logger.success(f"{account} | Done klaim point reff {amount}")
                        ...
                        try:
                            ...
                            if start_time is None and end_time is None and max_try > 0:
                                await blum.start()
                                logger.info(f"{account} | Gasss farming")
                                max_try -= 1
                                ...
                                logger.success(f"{account} | Berhasil klaim; Point total: {balance}")
                                max_try -= 1

                            elif end_time is not None and timestamp is not None:
                                sleep_duration = end_time - timestamp
                                logger.info(f"{account} | Proses farming sisa {format_duration(sleep_duration)}")
                                max_try += 1
                            ...
                            logger.error(f"{account} | Error farming: {e}")

                        await sleep(10)
                    except Exception as e:
                        logger.error(f"{account} | Error: {e}")
            except Exception as outer_e:
                logger.error(f"{account} | Sesi error: {outer_e}")
            finally:
                logger.info(f"{account} | Menghubungkan bos, 61 s")
                await sleep(61)
